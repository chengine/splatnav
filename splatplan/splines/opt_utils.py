...
from splatplan.splines.spline_utils import BezierCurve
from splatplan.splines.matrices import get_qp_matrices, project_point_into_polytope, get_single_spline_matrices, get_continuity_at_point
...
class SplineOptimizer():
    ...
    def optimize_bspline(self, polytopes, x0, xf, time_scales=None):
        ...
        if not torch.all( (A_x0 @ position_x0 - b_x0) <= 0.):
            ...
            projected_x0, success = project_point_into_polytope(A_x0, b_x0, position_x0)
            x0[0] = torch.tensor(projected_x0, dtype=torch.float32, device=self.device)

        if not torch.all( (A_xf @ position_xf - b_xf) <= 0.):
            print('Final position not in the last polytope!')
            projected_xf, success = project_point_into_polytope(A_xf, b_xf, position_xf)
            xf[0] = torch.tensor(projected_xf, dtype=torch.float32, device=self.device)
        ...
        if solver_success:
            ...
            # We'll rescale the time scales to be proper
    ...
    def optimize_bspline_coefficients(self, polytopes, x0, xf, time_scales=None):
        ...
        #print(bernstein_coefficients[:, 0])

        #Set up matrices
        A, b, C, d, Q = get_qp_matrices(bernstein_coefficients, polytopes, x0, xf)

        # progress_eval_points = torch.tensor([0., 1.], device=self.device)[None, :]
        ...
        # C_, d_ = get_continuity_at_point(bernstein_coefficients, x0, 0)
